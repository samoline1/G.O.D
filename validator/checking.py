...
def _get_and_update_config(
    ...
    update_model_info(config_dict, model)
    config = DictDefault(config_dict)
    return config


def _evaluate_test_set_loss(
    cfg: DictDefault, model: AutoModelForCausalLM, tokenizer: AutoTokenizer
):
    """
    what is DictDefault
    cfg which is a dictdefault already
    is then reassinged to cfg after transformed to a dictdefault 0.o
    """
    cfg = DictDefault(cfg)
...
def evaluate_finetune(
    ...
    # TODO: better name. What is config?? config for what?
    config = _get_and_update_config(
        dataset, finetuned_model, dataset_type, file_format, VALI_CONFIG_PATH
    )
    eval_results = _evaluate_test_set_loss(config, finetuned_model, tokenizer)
    return eval_results


def load_evaluation_dataset(cfg: DictDefault, tokenizer: AutoTokenizer):
    # TODO: It needs to be clear the flow. What is this prepared_path?
    # Where does it come from? What is cfg.
...
def log_dataset_and_model_info(
    eval_dataset, model: AutoModelForCausalLM, tokenizer: AutoTokenizer
):
...
def process_evaluation_batches(
    model: AutoModelForCausalLM, eval_dataloader: DataLoader, device: torch.device
):
    total_loss = 0
    num_batches = 0
